      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
              ...
              <button
                ...
                className={`flex items-center gap-2 px-6 py-3 rounded-full font-medium transition-all duration-300 ${
                  activeTab === tab.id
                    ...
                    : "bg-[#90CAF9] text-[#1565C0] border border-[#1565C0] hover:bg-[#43A047]/10"
                }`}
              >
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {tabContent[activeTab].map((item, index) => (
            <div key={index} className="bg-white border border-[#90CAF9] rounded-xl p-6 shadow-sm hover:shadow-lg transition-shadow duration-300">
              <div className="text-4xl mb-4">{item.icon}</div>
              <h3 className="text-xl font-bold text-[#1565C0] mb-3">{item.title}</h3>
              <p className="text-[#23272F] mb-4 leading-relaxed">{item.description}</p>
              {/* Rating */}
              <div className="flex items-center gap-2 mb-4">
                <div className="flex">
                  {[...Array(5)].map((_, i) => (
                    <Star key={i} className={`w-4 h-4 ${i < Math.floor(item.rating) ? 'text-[#43A047] fill-current' : 'text-[#90CAF9]'}`} />
                  ))}
                </div>
              ...
              <div className="flex flex-wrap gap-2">
                {item.benefits.map((benefit, idx) => (
                  <span key={idx} className="px-3 py-1 bg-[#90CAF9] text-[#1565C0] text-xs font-medium rounded-full border border-[#43A047]">
                    {benefit}
                  </span>
                ))}
              </div>
