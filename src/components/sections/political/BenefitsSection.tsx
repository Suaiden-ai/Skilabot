              ...
              <button
                ...
                className={`flex items-center gap-2 px-6 py-3 rounded-full font-medium transition-all duration-300 ${
                  activeTab === tab.id
                    ...
                    : "bg-white text-black border border-[#C62828] hover:bg-[#f8d7da]"
                }`}
              >
                <IconComponent className="w-5 h-5 text-[#183153]" />
                {tab.label}
              </button>
            );
          })}
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {tabContent[activeTab].map((item, index) => (
            <div key={index} className="bg-white border border-[#E3E6F0] rounded-xl p-6 shadow-sm hover:shadow-lg transition-shadow duration-300">
              ...
              <p className="text-black mb-4 leading-relaxed">{item.description}</p>
              {/* Rating */}
              ...
              <div className="flex flex-wrap gap-2">
                {item.benefits.map((benefit, idx) => (
                  <span key={idx} className="px-3 py-1 bg-[#C62828] text-white text-xs font-medium rounded-full">
                    {benefit}
                  </span>
                ))}
              </div>
